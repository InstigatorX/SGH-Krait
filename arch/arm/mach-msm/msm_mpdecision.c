...
#define MSM_MPDEC_CPU1_ON				12
#define MSM_MPDEC_CPU1_OFF				8
#define MSM_MPDEC_CPU1_ON_TIME			140
#define MSM_MPDEC_CPU1_OFF_TIME			190
...
static struct msm_mpdec_tuners {
    ...
    unsigned int cpu1_on;
    unsigned int cpu1_off;
    unsigned int cpu1_on_time;
    unsigned int cpu1_off_time;
    ...
    .cpu1_on = MSM_MPDEC_CPU1_ON,
    .cpu1_off = MSM_MPDEC_CPU1_OFF,
    .cpu1_on_time = MSM_MPDEC_CPU1_ON_TIME,
    .cpu1_off_time = MSM_MPDEC_CPU1_OFF_TIME,
};
...
static int mp_decision(void) {
    ...
    if (nr_cpu_online) {
        if ((nr_cpu_online < CONFIG_NR_CPUS) && (rq_depth >= msm_mpdec_tuners_ins.cpu1_on)) {
            if (total_time >= msm_mpdec_tuners_ins.cpu1_on_time) {
                new_state = MSM_MPDEC_UP;
                if (get_slowest_cpu_rate() <=  msm_mpdec_tuners_ins.idle_freq)
                    new_state = MSM_MPDEC_IDLE;
            }
        } else if ((nr_cpu_online > 1) && (rq_depth <= msm_mpdec_tuners_ins.cpu1_off)) {
            if (total_time >= msm_mpdec_tuners_ins.cpu1_off_time) {
	...
	/*
	pr_info(MPDEC_TAG"rq: %u, new_state: %i\n", rq_depth, new_state);
	*/
    return new_state;
}
...
out:
    if (state != MSM_MPDEC_DISABLED)
        queue_delayed_work_on(0, msm_mpdec_workq, &msm_mpdec_work,
...
static void msm_mpdec_late_resume(struct early_suspend *h) {
    ...
    was_paused = true;
    queue_delayed_work_on(0, msm_mpdec_workq, &msm_mpdec_work, 0);
...
show_one(cpu1_on, cpu1_on);
show_one(cpu1_off, cpu1_off);
show_one(cpu1_on_time, cpu1_on_time);
show_one(cpu1_off_time, cpu1_off_time);
show_one(suspend_frequency, suspend_frequency);
...
static ssize_t store_cpu1_on(struct kobject *a, struct attribute *b,
                   const char *buf, size_t count)
{
    unsigned int input;
    int ret;
    ...
    msm_mpdec_tuners_ins.cpu1_on = input;

    return count;
}

static ssize_t store_cpu1_off(struct kobject *a, struct attribute *b,
                   const char *buf, size_t count)
{
    unsigned int input;
    int ret;
    ...
    msm_mpdec_tuners_ins.cpu1_off = input;

    return count;
}

static ssize_t store_cpu1_on_time(struct kobject *a, struct attribute *b,
                   const char *buf, size_t count)
{
    unsigned int input;
    int ret;
    ...
    msm_mpdec_tuners_ins.cpu1_on_time = input;

    return count;
}

static ssize_t store_cpu1_off_time(struct kobject *a, struct attribute *b,
                   const char *buf, size_t count)
{
    unsigned int input;
    int ret;
    ...
    msm_mpdec_tuners_ins.cpu1_off_time = input;

    return count;
}

static ssize_t store_suspend_frequency(struct kobject *a, struct attribute *b,
                   const char *buf, size_t count)
{
    unsigned int input;
    int ret;
    ...
    msm_mpdec_tuners_ins.suspend_frequency = input;
...
define_one_global_rw(cpu1_on);
define_one_global_rw(cpu1_off);
define_one_global_rw(cpu1_on_time);
define_one_global_rw(cpu1_off_time);
define_one_global_rw(suspend_frequency);
...
static int __init msm_mpdec_init(void) {
    ...
    if (state != MSM_MPDEC_DISABLED)
        queue_delayed_work_on(0, msm_mpdec_workq, &msm_mpdec_work, HZ*25);
