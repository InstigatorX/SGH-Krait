...
static void hotplug_decision_work_fn(struct work_struct *work)
{
	 ...
	 * on this kernel is modified from original code, [strike]the iowait calculations were removed because 
	 * for the purpose of this driver we don't use that value and may cause extra overhead.[/strike] 
	 */
	...
	if (!hotplug_disabled) {
		if (hotplug_paused) {
			...
			} else if (!(delayed_work_pending(&hotplug_offline_work)) && !boostpulse_active && num_online_cpus() > 1) {
